        	...
        	<input list="nama_petugas" id="np" name="np" autocomplete="off" placeholder="Nama Petugas" class="form-control me-2" />
								<datalist id="nama_petugas">
									...
									<?php
										...
										<option value="<?=$_SESSION['nama_petugas']?>"><?=$_SESSION['nama_petugas']?></option>
									<?php
									}
									?>
								</datalist>
...
<!-- pencarian -->
	...
	<div class="row">
		...
		?>
			<div class="col-sm-8 text-white"><h5>Laporan Penjualan <?= $tanggal ?></h5></div>
			<?php 
			if($np<>""){
			?>
			<div class="col-sm-4 text-end text-white"><h5>Nama Petugas : <?= $np ?></h5></div>
			<?php
			}
			?>
